...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    <div
      ...
      aria-label="Theme"
      className={cn(
        "inline-flex items-center gap-0.5 rounded-lg border border-[hsl(var(--line))] bg-[hsl(var(--paper-2))] p-0.5",
        className,
      )}
    >
      {OPTIONS.map((opt) => {
        ...
        return (
          <button
            ...
            className={cn(
              "grid size-7 place-items-center rounded-md transition-colors duration-fast ease-out-quart",
              selected
